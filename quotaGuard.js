// El plan free de fútbol de BSD (sports.bzzoiro.com) permite 7.500
// requests/día. Este módulo cuenta cuántas llamadas hicimos HOY (se
// resetea solo al cambiar de fecha) y corta antes de llegar al límite,
// dejando un margen de seguridad — así nunca mandamos la request de más
// y arriesgamos la cuenta de nuevo.
//
...
// verdad más requests reales de las que quedan en un día. El contador
// sobrevive a los reinicios del mismo contenedor (no a un redeploy nuevo,
// que sí pisa el disco — para eso hace falta un disco persistente de
// Render o migrar a un servicio externo).
...
const SAFETY_MARGIN = 50; // dejamos de pedir un poco antes del límite real
...
// problema del lado de BSD mientras tanto.
...
// del lado de BSD), nuestro margen de seguridad no alcanza a verlo venir.
// dataSource.js llama a esto cuando la propia API contesta 429 con
// "code": "taster_exhausted", para que el resto del día este proceso
// corte de una en vez de seguir gastando llamadas reales que van a fallar
// igual.
...
class QuotaExceededError extends Error {
  constructor() {
    super(
      "Se alcanzó el límite diario de requests a la API externa (plan free de BSD). Probá de nuevo más tarde, o mirá datos ya cacheados mientras tanto."
    );
    this.name = "QuotaExceededError";
  }
}

// Distinto de QuotaExceededError: esto es un problema DE LA CUENTA (token
// inválido/revocado, cuenta suspendida), no de haber gastado el cupo del
// día. BSD lo reporta con un 401.
...
class AccountBlockedError extends Error {
  constructor(reason) {
    super(
      reason
        ? `La cuenta de BSD tiene un problema: "${reason}". Revisá https://sports.bzzoiro.com/ — esto no se arregla con reintentos.`
        : "La cuenta de BSD tiene un problema de acceso. Revisá https://sports.bzzoiro.com/."
    );
